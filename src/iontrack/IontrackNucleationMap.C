...
registerMooseObject("kookaburraApp", IontrackNucleationMap);

InputParameters
IontrackNucleationMap::validParams()
{
  ...
  params.addRequiredParam<Real>("angle", "DiscreteNucleationInserter user object");
...
IontrackNucleationMap::IontrackNucleationMap(const InputParameters & parameters)
...
void
IontrackNucleationMap::initialize()
{
...
void
IontrackNucleationMap::threadJoin(const UserObject & y)
{
  // if the map needs to be updated we merge the maps from all threads
  if (_rebuild_map)
  {
    const IontrackNucleationMap & uo = static_cast<const IontrackNucleationMap &>(y);
    _nucleus_map.insert(uo._nucleus_map.begin(), uo._nucleus_map.end());
  }
}

void
IontrackNucleationMap::meshChanged()
{
  _mesh_changed = true;
}

const std::vector<Real> &
IontrackNucleationMap::nuclei(const Elem * elem) const
{
