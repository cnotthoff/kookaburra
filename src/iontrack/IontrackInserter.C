...
#include "IontrackInserter.h"
#include "libmesh/parallel_algebra.h"

#include "libmesh/quadrature.h"

registerMooseObject("kookaburraApp", IontrackInserter);

InputParameters
IontrackInserter::validParams()
{
...
IontrackInserter::IontrackInserter(const InputParameters & parameters)
  : DiscreteNucleationInserterBase(parameters),
    ...
    _time_dep_stats(getParam<bool>("time_dependent_statistics")),_N_ions(0),_N_ions_old(0)
{
}

void
IontrackInserter::initialize()
{
  //const Real random = getRandomReal();
  //std::cerr<<"test="<<random<<std::endl;
  //unsigned int dim = _fe_problem.mesh().dimension();
  //std::cerr<<"test="<<dim<<std::endl;

  //BoundingBox bbox = _fe_problem.mesh().getInflatedProcessorBoundingBox();
  //std::cerr<<"test_min="<<bbox.min()<<std::endl;
  //std::cerr<<"test_max="<<bbox.max()<<std::endl;
...
void
IontrackInserter::threadJoin(const UserObject & y)
{
  // combine _local_nucleus_list entries from all threads on the current process
  const IontrackInserter & uo = static_cast<const IontrackInserter &>(y);
  ...
  // integrate total number of ions
  _N_ions += uo._N_ions;
}

void
IontrackInserter::finalize()
{
  ...
  // gather the total number of ions
  gatherSum(_N_ions);
  _N_ions_old+=_N_ions;
  _update_required = _changes_made.first > 0 || _changes_made.second > 0;
}

registerMooseObject("kookaburraApp", IonFluencePostprocessor);
//IonFluence
defineLegacyParams(IonFluencePostprocessor);

InputParameters
IonFluencePostprocessor::validParams()
{
...
IonFluencePostprocessor::IonFluencePostprocessor(const InputParameters & parameters)
  : GeneralPostprocessor(parameters),
    _fluence(0.0),
    _inserter(getUserObject<IontrackInserter>("inserter"))//,
...
void
IonFluencePostprocessor::initialize()
{
}

void
IonFluencePostprocessor::execute()
{
  //_sum = _sum_old + _pps_value;
  _fluence = _inserter.getFluence();
}

Real
IonFluencePostprocessor::getValue()
{
  return _fluence;
}

void
IontrackInserter::addNucleus(unsigned int & qp)
{
